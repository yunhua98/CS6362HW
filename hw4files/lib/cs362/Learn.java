...
public class Learn {
	...
	public static void main(String[] args) throws IOException {
		...
		if (CommandLineUtilities.hasArg("gaussian_kernel_sigma"))
   		...
   		// Setting the lambda value for lambda means clustering
   		opts.cluster_lambda = 0.0;
   		if (CommandLineUtilities.hasArg("cluster_lambda"))
   			opts.cluster_lambda = CommandLineUtilities.getOptionValueAsFloat("cluster_lambda");

   		// Setting the training iterations for lambda means clustering
		opts.clustering_training_iterations = 10;
		if (CommandLineUtilities.hasArg("clustering_training_iterations"))
    		opts.clustering_training_iterations = CommandLineUtilities.getOptionValueAsInt("clustering_training_iterations");
	...
	private static Predictor train(List<Instance> instances, String algorithm, TrainingOpts opts) {
	    ...
	    if (algorithm.equalsIgnoreCase("lambda_means")) {
	    	LambdaMeansPredictor predictor = new LambdaMeansPredictor(opts.cluster_lambda, opts.clustering_training_iterations);
	    	predictor.train(instances);
	    	return predictor;
	    }
